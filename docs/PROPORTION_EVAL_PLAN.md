...
> **Note on paths.** The repo was reorganised after this work finished:
> `eval/` split into `evaluation/` (live scripts), `docs/` + `docs/history/`
> (write-ups), and `archive/` (superseded phases); `models/data_utils.py` is
> now `models/data.py` and `models/lora_gate/` is `models/lora/`. Paths in the
> progress log below are as they were at the time. Current layout: `README.md`.
...
| Folds | **6**, held-out sets in `evaluation/folds.json` | Balanced on raw rows (1888-2359) with the 5 DT-rich works spread across 5 folds so no fold's training set is starved of DT (train-side DT 469-638). |
...
| Bias policy | **Disclose, never correct** | Matches the decision in `docs/history/bert_cascade_evolution.md`. |
...
- [x] Compute balanced 6-fold assignment -> `evaluation/folds.json`
- [x] Write this plan of record
- [x] P0: add `nonjunk_3way` + `full4way` stages and 3/4-way prompts to `models/lora/train.py`
...
- [x] P4: `evaluation/evaluate_proportions.py` + `proportion_metrics.json` + figures
- [x] P5: write-up — `docs/PROPORTION_EVAL_RESULTS.md`
...
- 2026-08-23: **COMPLETE.** Results in `docs/PROPORTION_EVAL_RESULTS.md`.
...
- 2026-08-23: **P2 attempt 1 failed, two bugs, both fixed.**
  1. Comma-delimited `--holdout-work` split three real titles that CONTAIN
     ...
     Fixed by adding `--holdout-fold`, which reads titles from evaluation/folds.json
...
- 2026-08-23: P0 done. `models/lora/train.py` now takes `--stage`
  ...
  `models/data.py` and per-work mix reporting to the run report.
  Module-level `HOLDOUT_WORK`/`STAGE` kept as defaults so `archive/bert_phase/ensemble_gate.py`
  and `archive/bert_phase/backfill_gate_proportions.py` still import cleanly.
...
- 2026-08-23: **P1 COMPLETE.** Full search ranking by dev-text (Reign of Law)
  ...
  **The 4-way stage-2 idea is rejected.** Every full4way run loses on accuracy
  ...
  the same effect documented in docs/history/bert_cascade_evolution.md's original
...
- 2026-08-23: P1 search burst submitted — 8 configs x 2 seeds (42/7) on fold4:
  ...
  recreatable (see `models/lora/submit.slurm` for the same pattern; the
