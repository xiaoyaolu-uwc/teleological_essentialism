...
Usage:
    python3 evaluation/run_cascade_folds.py --gate-prefix gate_fold --s2-prefix s2_fold \
        --s2-stage nonjunk_3way --s2-prompt S2_structured --text-column text \
        --out evaluation/results/proportions/per_row_predictions.csv
"""
import argparse
import csv
import json
...
from models.data import load_train_pool
from models.labels import STAGE_LABELS
...
def main():
    ...
    ap.add_argument("--folds", default="evaluation/folds.json")
    ...
    ap.add_argument("--gate-suffixes", nargs="+", default=[""],
                     help="Checkpoint suffixes to ENSEMBLE over, e.g. '' '_s7' averages "
                          ...
                          "evaluation/calibrate_gate_threshold.py). One suffix = no ensemble.")
    ...
    ap.add_argument("--out", default="evaluation/results/proportions/per_row_predictions.csv")
    args = ap.parse_args()
